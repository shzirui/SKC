...
def _normalize_lora_adapter_key(key: str) -> str:
    return key.replace(".lora_A.default.weight", ".lora_A.weight").replace(".lora_B.default.weight", ".lora_B.weight")


def _extract_lora_params_from_hf_checkpoint(hf_path: str) -> OrderedDict:
    ...
    if os.path.isfile(index_path):
        ...
        for key, shard_name in weight_map.items():
            if "lora_" in key:
                by_file.setdefault(shard_name, []).append(key)
        for shard_name, keys in by_file.items():
            shard_path = os.path.join(hf_path, shard_name)
            with safe_open(shard_path, framework="pt", device="cpu") as f:
                for key in keys:
                    lora_params[_normalize_lora_adapter_key(key)] = f.get_tensor(key).detach().cpu()
        return lora_params

    if os.path.isdir(hf_path):
        for filename in sorted(os.listdir(hf_path)):
            if not filename.endswith(".safetensors"):
                continue
            shard_path = os.path.join(hf_path, filename)
            with safe_open(shard_path, framework="pt", device="cpu") as f:
                for key in f.keys():
                    if "lora_" in key:
                        lora_params[_normalize_lora_adapter_key(key)] = f.get_tensor(key).detach().cpu()
...
def _assert_nonempty_lora_adapter(adapter_model_path: str) -> int:
    if not os.path.isfile(adapter_model_path):
        raise RuntimeError(f"LoRA adapter_model.safetensors was not created: {adapter_model_path}")
    with safe_open(adapter_model_path, framework="pt", device="cpu") as f:
        num_keys = len(f.keys())
    if num_keys <= 0:
        raise RuntimeError(f"LoRA adapter_model.safetensors has no tensor keys: {adapter_model_path}")
    return num_keys
...
def _parse_global_step_dir(step_dir: str) -> Optional[int]:
    name = os.path.basename(os.path.dirname(step_dir))
    prefix = "global_step_"
...
def _delete_lora_artifacts_after_save(actor_local_path: str, keep_resume: int, log_fn=None):
    actor_path = os.path.abspath(actor_local_path)
    current_step_dir = os.path.dirname(actor_path)
    exp_dir = os.path.dirname(current_step_dir)

    hf_dir = os.path.join(actor_path, "huggingface")
    if os.path.isdir(hf_dir):
        shutil.rmtree(hf_dir, ignore_errors=True)
        if log_fn:
            log_fn(f"Deleted LoRA temporary HuggingFace export: {hf_dir}")
    ...
    if os.path.isdir(exp_dir):
        for name in os.listdir(exp_dir):
            step_dir = os.path.join(exp_dir, name)
            if not os.path.isdir(step_dir):
                continue
            step = _parse_global_step_dir(os.path.join(step_dir, "actor"))
            actor_dir = os.path.join(step_dir, "actor")
            if step is not None and os.path.isdir(actor_dir):
                step_actor_paths.append((step, actor_dir))
    step_actor_paths.sort(key=lambda item: item[0])
    keep_steps = {step for step, _ in step_actor_paths[-keep_resume:]} if keep_resume > 0 else set()
    resume_prefixes = ("model_world_size_", "optim_world_size_", "extra_state_world_size_")
    deleted = 0
    for step, actor_dir in step_actor_paths:
        if step in keep_steps:
            continue
        for filename in os.listdir(actor_dir):
            if filename.endswith(".pt") and filename.startswith(resume_prefixes):
                ...
                try:
                    ...
                    if log_fn:
                        log_fn(f"Failed to delete old LoRA resume state: {path}")
    if log_fn and deleted:
        log_fn(f"Deleted {deleted} old LoRA resume state files; keep_resume_steps={sorted(keep_steps)}")
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            if self.config.actor.get("use_ewc", False):
                ewc_state_path = self.config.actor.get("ewc_state_path", None)
                if not ewc_state_path:
                    raise ValueError("actor_rollout_ref.actor.use_ewc is true but ewc_state_path is empty")
                if not os.path.exists(ewc_state_path):
                    raise FileNotFoundError(f"EWC state file not found: {ewc_state_path}")
                ewc_state = torch.load(ewc_state_path, map_location="cpu")
                if not isinstance(ewc_state, dict) or "theta_star" not in ewc_state or "fisher" not in ewc_state:
                    raise ValueError("EWC state must be a dict containing 'theta_star' and 'fisher'")
                self.actor.ewc_state = ewc_state
                self.actor.use_ewc = True
                self.actor.ewc_coef = float(self.config.actor.get("ewc_coef", 0.0))
                if self.rank == 0:
                    print(f"Loaded EWC state from {ewc_state_path}")
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        if self._is_lora and hasattr(getattr(self, "actor_module", self.actor_module_fsdp), "peft_config"):
            ...
            if fsdp_version(self.actor_module_fsdp) > 0:
                ...
                if dist.get_rank() == 0 and not lora_params:
                    hf_path = os.path.join(local_path, "huggingface")
                    lora_params = _extract_lora_params_from_hf_checkpoint(hf_path)
                if dist.get_rank() == 0:
                    if not lora_params:
                        raise RuntimeError(
                            "LoRA is enabled, but no LoRA tensor was collected from FSDP or the HuggingFace checkpoint."
                        )
                    ...
                    num_lora_keys = _assert_nonempty_lora_adapter(adapter_model_path)
                    log_with_rank(
                        f"[rank-{self.rank}]: Saved LoRA adapter to: {lora_save_path} ({num_lora_keys} tensors)",
                        rank=dist.get_rank(),
                        logger=logger,
                        log_only_rank_0=True,
                    )
                    if _as_bool(self.config.actor.get("prune_lora_artifacts", False)):
                        keep_resume = int(self.config.actor.get("lora_resume_ckpt_to_keep", 1))

                        def _log_prune(message):
                            log_with_rank(
                        ...
                        _delete_lora_artifacts_after_save(local_path, keep_resume=keep_resume, log_fn=_log_prune)
