...
def get_base_subgoal_name(name: str) -> str:
    ...
    Input: "Click Menu" -> Output: "Click Menu"
    """
    return re.sub(r"\s*\(Part\s*\d+\)$", "", name).strip()

async def async_score_recursive_step_multi_turn(
    task_instruction: str, 
    task_root_path: str,
    all_subgoals: List[Dict],
    current_idx: int,
    current_outcome_reward: float,
    async_call_llm_fn  # 传入的异步回调函数
) -> Optional[Dict]:
    """
    核心打分逻辑 (异步版)
    """
    # -----------------------------------------------------------
    # 1. Context 提取 (完全复用原有逻辑)
    # -----------------------------------------------------------
    ...
    while temp_next_idx < len(all_subgoals):
        ...
        if n_base != current_base_name:
            next_subgoal_str = f"Goal: {n_base}\n  Reasoning: {n_item.get('reasoning', 'N/A')}"
            break
        temp_next_idx += 1

    evidence_subgoal = all_subgoals[current_idx-1].get('subgoal_name', 'Task Start') if current_idx > 0 else "Task Start"

    # -----------------------------------------------------------
    # 2. Evidence 提取 (完全复用原有逻辑)
    # -----------------------------------------------------------
    raw_steps_data = current_sg_info.get('steps_data', [])
    evidence_b64 = None
    evidence_text = "Unknown"
    current_group_pure_steps = []
    start_b64 = None
    ...
    if raw_steps_data:
        ...
        if "[PREVIOUS CONTEXT]" in first_step.get('content', ''):
            ...
            current_group_pure_steps = raw_steps_data[1:] 
        else:
            current_group_pure_steps = raw_steps_data

    start_state_b64 = None 
    end_state_b64 = None   
    
    if current_group_pure_steps and task_root_path:
        if current_group_pure_steps[0].get('image'):
            start_state_b64 = encode_image_to_base64(os.path.join(task_root_path, current_group_pure_steps[0]['image']))
        if current_group_pure_steps[-1].get('next_image'):
            end_state_b64 = encode_image_to_base64(os.path.join(task_root_path, current_group_pure_steps[-1]['next_image']))
    ...
    **YOUR CONTEXT:**
    ...
    - **Subgoal Trace**: Context of Previous, Current, and Next subgoals.
    ...
    **HOW TO USE 'CURRENT GROUP OUTCOME' (Global Constraint):**
    ...
    user_content_p1.append({
        "type": "text",
        "text": f"""
    **PHASE 1: STEP-LEVEL CRITIC (INSTRUCTIONS)**
    For EACH step, evaluate and assign a score:
...
"""
    ...
    for s_idx in sibling_indices:
        ...
        for i, step in enumerate(filtered_steps):
            ...
            if is_current_part:
                ...
                if i == 0:
                    user_content_p1.append({"type": "text", "text": f"\n[START STATE] (Current Group Start):"})
                b64 = encode_image_to_base64(img_path)
                if b64:
                    user_content_p1.append({"type": "image_url", "image_url": {"url": f"data:image/png;base64,{b64}"}})
                user_content_p1.append({"type": "text", "text": step_display_text})
                
                if i == len(filtered_steps) - 1:
                    if s_idx == sibling_indices[-1]:
                        ...
                        if b64_end:
                            ...
                            user_content_p1.append({"type": "image_url", "image_url": {"url": f"data:image/png;base64,{b64_end}"}})
            elif is_immediate_next and i == 0:
                user_content_p1.append({"type": "text", "text": f"\n[FINAL RESULT STATE] (Result of Current Group):"})
                b64 = encode_image_to_base64(img_path)
                if b64:
                    user_content_p1.append({"type": "image_url", "image_url": {"url": f"data:image/png;base64,{b64}"}})
                user_content_p1.append({"type": "text", "text": f"{step_display_text}"})
            else:
                user_content_p1.append({"type": "text", "text": f"\n[Context Text Only]: {step_display_text}"})
    
    target_indices_str = ", ".join(map(str, target_step_indices))

    if user_content_p1 and 'text' in user_content_p1[0]:
        original_text = user_content_p1[0]['text']
        ...
        user_content_p1[0]['text'] = original_text.replace(old_phrase, new_phrase)

    user_content_p1.append({
        "type": "text",
        "text": f"""
    \n## Output Format (JSON) for PHASE 1
    {{
    ...
    conversation_history.append({"role": "system", "content": system_prompt_p1})
    conversation_history.append({"role": "user", "content": user_content_p1})
    ...
    try:
        phase1_result = json.loads(clean_json_response(response_content))
        if "step_critic" in phase1_result:
            original_list = phase1_result["step_critic"]
            cleaned_list = [
                item for item in original_list 
                if isinstance(item, dict) and item.get("step_index") in target_step_indices
            ]
            phase1_result["step_critic"] = cleaned_list
        conversation_history.append({"role": "assistant", "content": response_content})
        final_merged_result.update(phase1_result)
    except Exception as e:
        print(f"[PRM Score] Phase 1 parse error: {e}")
        return None

    # === Phase 2: Current Group Conclusion ===
    user_content_p2 = f"""
    Great. Now proceed to **PHASE 2: Current Group Conclusion**.

    Here is the **Subgoal Trace** for Context:
    **1. Previous Group Subgoal:**
    {prev_subgoal_str}

    **2. Current Group Subgoal:**
    Subgoal: {current_base_name}
    Reasoning: {current_reasoning}

    **3. Next Group Subgoal:**
    {next_subgoal_str}
    Evaluate the Group's total contribution to the **Global Task** based on the Step-Level Critic you just performed and the Subgoal Trace.
    ...
    **PHASE 2 INSTRUCTIONS:**
    ...
    \n## Output Format (JSON) for PHASE 2
    {{
    ...
    conversation_history.append({"role": "user", "content": user_content_p2})
    
    # 🌟 异步调用 Phase 2
    response_content = await async_call_llm_fn(conversation_history, max_tokens=1024)
    if not response_content: return None
    
    try:
        phase2_result = json.loads(clean_json_response(response_content))
        conversation_history.append({"role": "assistant", "content": response_content})
        final_merged_result.update(phase2_result)
    except Exception as e:
        print(f"[PRM Score] Phase 2 parse error: {e}")
        return None

    # === Phase 3: Previous Group Judgment ===
    user_content_p3 = []
    evidence_prompt_text = f"""
    Finally, proceed to **PHASE 3: Previous Group Judgment**.
    Verify if the Previous Group achieved its subgoal based on the provided EVIDENCE.
    ...
    **PHASE 3 INSTRUCTIONS:**
       ...
       - **Step 2: Score**: 
          ...
          - **2. Partial Success (0.6 - 0.9)**: The state shows **PARTIAL COMPLETION** or progress.
          - **3. Valuable Failure (0.3 - 0.5)**: The subgoal **FAILED**, BUT the Last Action performed was **CORRECT/VALUABLE** (Salvage value).
    ...
    ## Output Format (JSON) for PHASE 3
    {{
    ...
    if evidence_b64:
        user_content_p3.append({"type": "text", "text": f"""## [EVIDENCE FOR PHASE 3] PREVIOUS GROUP END STATE:\n**Previous Subgoal:** "{evidence_subgoal}" """})
        user_content_p3.append({"type": "image_url", "image_url": {"url": f"data:image/png;base64,{evidence_b64}"}})
        user_content_p3.append({"type": "text", "text": f"""\n**Last Action Performed:** {evidence_text}"""})
        user_content_p3.append({"type": "image_url", "image_url": {"url": f"data:image/png;base64,{start_b64}"}})
    else:
        user_content_p3.append({"type": "text", "text": f"""## [EVIDENCE FOR PHASE 3] PREVIOUS GROUP END STATE:\n**Previous Subgoal:** "{evidence_subgoal}" """})

    user_content_p3.append({"type": "text", "text": evidence_prompt_text})
    conversation_history.append({"role": "user", "content": user_content_p3})
    ...
    try:
        phase3_result = json.loads(clean_json_response(response_content))
        final_merged_result.update(phase3_result)
    except Exception as e:
        print(f"[PRM Score] Phase 3 parse error: {e}")
        return None
